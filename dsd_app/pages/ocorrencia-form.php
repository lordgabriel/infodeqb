...
<?php endif; ?>
  ...
  <div class="card-title" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
    <span>Turmas e Horas de Contacto</span>
    <a href="uc-form.php?id=<?= (int)$ocor['uc_id'] ?>" class="btn btn-secondary btn-sm" style="font-size:12px">
      <i class="fas fa-edit me-1"></i>Editar UC
    </a>
  </div>
  ...
  ?>
  <div style="margin-bottom:10px;padding:8px 12px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:6px;font-size:12px;color:var(--gray-600)">
    ...
    <?php foreach ($tipos as [$k, $label, $ucField]):
      ...
      $isDiffUC = abs($hT - $hUC) > 0.001;
    ?>
    <span style="margin-left:8px;<?= $isDiffUC ? 'color:var(--orange);font-weight:600' : '' ?>"><?= $k ?>: <?= fmt($hUC, 2) ?>h<?php if ($isDiffUC): ?> <i class="fas fa-exclamation-triangle" title="Esta ocorrência usa <?= fmt($hT, 2) ?>h"></i><?php endif; ?></span>
    <?php endforeach; ?>
  </div>
