...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        try {
            $db->prepare("DELETE FROM infodeqb_dsd_carreira WHERE id=?")->execute([(int)$_POST['delete_id']]);
            flash('Removido.');
        } catch (Exception $e) {
        ...
        if (!$des) { flash('Designação obrigatória.', 'error'); }
            ...
            try {
                if ($id) {
                    $db->prepare("UPDATE infodeqb_dsd_carreira SET designacao=?,ordem=? WHERE id=?")->execute([$des,$ord,$id]);
                } else {
                    $chk = $db->prepare("SELECT id FROM infodeqb_dsd_carreira WHERE designacao=? LIMIT 1");
                    $chk->execute([$des]);
                    if ($chk->fetch()) flash('Já existe uma carreira com essa designação.', 'error');
                    else $db->prepare("INSERT INTO infodeqb_dsd_carreira (designacao,ordem) VALUES (?,?)")->execute([$des,$ord]);
                }
                flash('Guardado.');
            } catch (Exception $e) { flash('Erro: '.$e->getMessage(),'error'); }
        }
    }
    header('Location: carreiras.php'); exit;
}
$rows = $db->query("SELECT * FROM infodeqb_dsd_carreira ORDER BY ordem, designacao")->fetchAll();
require_once __DIR__ . '/../includes/header.php';
?>
<div class="page-header">
  <div><div class="page-title"><i class="fas fa-graduation-cap me-1"></i>Carreiras</div>
