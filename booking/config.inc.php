 ...
 *********************************/
$mrbs_admin = "Direção DEQB";
$mrbs_admin_email = 'jfeyo@fe.up.pt';
...
$mrbs_company = "Departamento de Engenharia Química e Biológica";   // This line must always be uncommented ($mrbs_company is used in various places)
...
$mrbs_company_url = "https://deq.fe.up.pt/infodeq";
...
// notification.
$url_base = "https://deq.fe.up.pt/infodeqb/booking/";
...
$auth['shibboleth']['site_uri'] = "https://deq.fe.up.pt/infodeqb/booking/";
...
//com atributo de afilização (original)
//$auth['shibboleth']['user_roles'] = array(
    //    'eduPersonPrimaryAffiliation' => array('staff', 'student', 'alum', 'faculty')
//);

//Som atributo de afilização (original)
$auth['shibboleth']['user_roles'] = [
    'eppn' => [
        ['type' => 'regex', 'value' => '/@up\.pt$/']
    ]
];
...
$auth['shibboleth']['admin_roles'] = array(
    'eppn' => array('up356946@up.pt','up444525@up.pt','up424064@up.pt','up448105@up.pt', 'up701282@up.pt', 'up619087@up.pt')
);
...
$mail_settings['recipients'] = 'jfeyo@fe.up.pt';
...
/*******************************************************************************
 * DEV ONLY — Simulação Shibboleth para ambiente local (XAMPP)
 * Este bloco injeta atributos $_SERVER falsos a partir da sessão PHP gerida
 * por dev_login.php.  Só actua em localhost — não tem efeito em produção.
 * REMOVER (ou manter — é inofensivo em prod porque a condição nunca é true)
 ******************************************************************************/
if (in_array($_SERVER['SERVER_NAME'] ?? 'localhost', ['localhost', '127.0.0.1', '::1']))
{
    $dev_session_name = 'DEV_SHIB';
    // Guardar e restaurar o nome de sessão para não interferir com o MRBS
    $prev_session_name = session_name($dev_session_name);
    if (session_status() === PHP_SESSION_NONE)
    {
        session_start();
    }
    if (!empty($_SESSION['dev_shib_user']))
    {
        $dev_u = $_SESSION['dev_shib_user'];
        $_SERVER['Mail']        = $dev_u['mail'];
        $_SERVER['DisplayName'] = $dev_u['name'];
        $_SERVER['eppn']        = $dev_u['eppn'];
    }
    // Fechar esta sessão antes que o MRBS inicie a sua própria
    session_write_close();
