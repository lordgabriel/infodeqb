...
function get_fieldset_general($data)
{
  ...
  // Administradores da sala por número UP (apenas admins globais podem editar)
  if (is_admin())
  {
    $room_admins_list = implode(', ', get_room_admins((int)$data['id']));
    $field = new FieldInputText();
    $field->setLabel('Administradores da sala (nº UP)')
          ->setLabelAttribute('title', 'Números UP separados por vírgula (ex: 356946, 444525). Estes utilizadores podem aprovar e gerir reservas desta sala.')
          ->setControlAttributes(array(
              'name'        => 'room_admins_up',
              'value'       => $room_admins_list,
              'placeholder' => 'ex: 356946, 444525'));
    $fieldset->addElement($field);
  }
