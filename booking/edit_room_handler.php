...
if (empty($errors))
{
  ...
  else
  {
    ...
    // Guardar os administradores da sala por número UP (fora da transacção principal)
    // Só admins globais podem definir admins de sala
    if (is_admin() && isset($room_admins_up))
    {
      $up_numbers = array_map('trim', explode(',', $room_admins_up));
      // Filtrar entradas inválidas (só aceitar dígitos)
      $up_numbers = array_filter($up_numbers, function($v) {
        return preg_match('/^\d+$/', $v);
      });
      set_room_admins((int)$room, array_values($up_numbers));
    }
